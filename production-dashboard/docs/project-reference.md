...
### 개요
- 라인: A/B 2개 라인
- 역할: 하판+상판 결합 후 최종 정렬 검사
- 위치: 프린팅 → 로우슬리터 → **조립/검사** (공정 마진 최종 확인)

### 측정 데이터
- CSV: 샘플검사 CSV (~206줄, UTF-16LE)
- 3개 섹션: 타발홀 / 양면(좌우) / 양면(상하)
- 각 섹션 Row 1~12 전체 측정
- 허용범위: NG ±0.15mm

### 핵심 개념: 잔여율
- 잔여율(%) = (NG_LIMIT - |편차|) / NG_LIMIT × 100
- Rev.8의 MarginPage에서 도입된 개념
- 잔여율 <20% = 위험, 다음 SET에서 NG 가능성

### ⚠️ 보정 방식 차이
- 프린팅/로우슬리터: 전역 보정 (전체 Row 동시 이동)
- 자동조립기: **Row 1~12 각각 좌우/상하 독립 보정 가능**
- AI가 두 경로 중 적절한 것을 제안
...
### 8.1 측정 위치
프린팅 CSV에서 Row별 편차는 시트의 4코너를 기준으로 측정한다:
- **좌상 (TL)**: 시트 왼쪽 위
- **우상 (TR)**: 시트 오른쪽 위
- **좌하 (BL)**: 시트 왼쪽 아래
- **우하 (BR)**: 시트 오른쪽 아래
...
### 8.3 Row별 특성
- Row 1, 12 (양 끝): Q값(회전) 영향 최대
- Row 6, 7 (가운데): Q값 영향 최소, 순수 오프셋만 반영
- 이 특성이 시뮬레이션의 positionFactor 계산에 반영됨:
  `positionFactor = (row - center) / center`

### 8.4 인쇄별 측정 포인트
- 카본: 4코너 기준, 타발 기준 좌우/상하
- 절연: 4코너 기준, 카본 대비 상대 위치
- 실버: 카본 기준 대비 상대 위치

### 8.5 측정값 → 대시보드 매핑
| CSV 컬럼 | 대시보드 표시 | 단위 |
|----------|-------------|------|
| 거리_타발_카본_좌우 | ←좌/우→ 편차 | mm |
| 거리_타발_카본_상하 | ↑상/하↓ 편차 | mm |
| 거리_타발_절연_좌우 | (절연 탭) ←좌/우→ | mm |
| 거리_타발_절연_상하 | (절연 탭) ↑상/하↓ | mm |

## 수동조립기 공정

### 개요
- 설비 수: 4대 (수동조립기 #1~#4)
- 조립 방식: 작업자가 수동으로 상판+양면(하판) 결합
- 편차 원인: 설비 정밀도 + 작업자 조립 습관
...
### CSV 파일명 (예상)
```
250214_수동조립1_가속3차_SET_3.csv
250214_수동조립2_가속3차_PRD_5.csv
```
- 파일명에 설비번호(1~4) 포함
- 실제 CSV 포맷은 미정, 현재 샘플 데이터로 운영

### 편차 유형
| 유형 | 원인 | 대응 |
|------|------|------|
| 설비 편차 | 지그 마모/이탈 | 지그 교체/정렬 |
| 작업자 편차 | 조립 습관 차이 | 교육/표준화 |
| 혼합 편차 | 설비+작업자 | 분리 분석 필요 |

### 라인 구조
- 수동조립기 4대가 병렬 운영 (A/B 라인 개념 없음)
- 각 설비에 작업자 1명 배정
- 교대 시 작업자가 바뀌므로 작업자ID 추적 필요
