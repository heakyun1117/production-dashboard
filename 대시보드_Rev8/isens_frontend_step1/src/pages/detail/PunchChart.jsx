...
export default function PunchChart({ rows }) {
  const storeT = useThresholdStore((s) => s.thresholds.assembly);
  const CHECK = storeT.check;
  const NG = storeT.ng;
  ...
  if (data.length === 0) {
    return <Typography variant="body2" sx={{ color: "text.secondary" }}>Punch 데이터 없음</Typography>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
        ...
        <Bar dataKey="L" name="타발홀L" maxBarSize={24}>
          {data.map((d, i) => <Cell key={i} fill={barColor(d.L, CHECK, NG)} fillOpacity={0.85} />)}
        </Bar>
        <Bar dataKey="R" name="타발홀R" maxBarSize={24}>
