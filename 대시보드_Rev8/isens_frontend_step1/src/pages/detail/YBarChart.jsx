...
export default function YBarChart({ rows }) {
  const storeT = useThresholdStore((s) => s.thresholds.assembly);
  const CHECK = storeT.check;
  const NG = storeT.ng;
  ...
  const data = useMemo(() => {
    ...
    return rows.map((r, i) => ({
      row: r.Row ?? i + 1,
      L: asNumber(r["상하치우침L"]) ?? 0,
      C: asNumber(r["상하치우침C"]) ?? 0,
      R: asNumber(r["상하치우침R"]) ?? 0,
    }));
  }, [rows]);
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
        ...
        <ReferenceLine y={NG} stroke={COLOR_NG} strokeWidth={1.5} label={{ value: "NG", position: "right", fontSize: 10, fill: COLOR_NG }} />
        ...
        <Bar dataKey="L" name="상하치우침L" maxBarSize={20}>
          {data.map((d, i) => <Cell key={i} fill={barColor(d.L, CHECK, NG)} fillOpacity={0.85} />)}
        </Bar>
        <Bar dataKey="C" name="상하치우침C" maxBarSize={20}>
          {data.map((d, i) => <Cell key={i} fill={barColor(d.C, CHECK, NG)} fillOpacity={0.85} />)}
        </Bar>
        <Bar dataKey="R" name="상하치우침R" maxBarSize={20}>
